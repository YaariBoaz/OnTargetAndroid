...
export class RecommendationsService {
  ...
  getRecommendation(shots: any[], centerCord: { X: number; Y: number; }) {
    let p = 0;
    ...
    const res = new Recommendation(p, s.Recommendation);

    return res;
  }

  isInSquare(sq: Square, list: any[]) {
    let count = 0;
  ...
  inSquare(sq: Square, hit: { x: number; y: number; }) {
